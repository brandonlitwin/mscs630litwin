...
public class AEScipher {
   ...
   *   roundKeys: A String array of the roundkeys
   *   pTextMatrix: The plaintext converted into a matrix
   *   XORRoundKey: The matrix after doing the addRoundKey step
   *   nibbleSubstitutionKey: The matrix after doing the nibble substitution
   *   shiftRows: The matrix after shifting the rows
   *   mixColumns: The matrix after mixing the columns
   *   substringVal: The current placeholder for the substring of the keyHex
   *
   * Return value: cipherText, a String containing the resulting ciphertext
   */
  public static String AES(String pTextHex, String keyHex) {
    ...
    for (int i = 0; i < roundKeys.length; i++) {
      if (i == 0) {
        ...
        XORRoundKey = AESStateXOR(shiftRows, roundKeys[i]);
      } else {
        XORRoundKey = AESStateXOR(mixColumns, roundKeys[i]);
      }
      if (i < 10) {
    ...
    for (int i = 0; i < XORRoundKey.length; i++) {
      for (int j = 0; j < XORRoundKey.length; j++) {
        if (Integer.toHexString(XORRoundKey[j][i]).length() == 1) {
          cipherText += "0" + Integer.toHexString(XORRoundKey[j][i]);
        } else {
          cipherText += Integer.toHexString(XORRoundKey[j][i]);
        }
      }
    }
   ...
   *
   * Return value: XORMatrix, a 2D array containing the resulting 4x4 matrix
   *
   */
   ...
   *
   * Return value: nibbleSubMatrix, a 2D array containing the resulting 4x4
   *               matrix
   *
   */
   ...
   *   inStateHex: A 4x4 matrix containing the current hex
   *
   * Return value: shiftRowMatrix, a 2D array containing the resulting 4x4
   *                               matrix
   */
   ...
   *   inStateHex: A 4x4 matrix containing the current hex
   *
   * Return value: mixColumnsMatrix, a 2D array containing the resulting 4x4
   *               matrix
   */
  public static int[][] AESMixColumn(int[][] inStateHex) {
    int[][] mixColumnsMatrix = new int[MATRIX_SIZE][MATRIX_SIZE];
    for (int i = 0; i < inStateHex.length; i++) {
      mixColumnsMatrix[0][i] =
        multiply2[inStateHex[0][i]] ^ multiply3[inStateHex[1][i]]
        ^ inStateHex[2][i] ^ inStateHex[3][i];
      mixColumnsMatrix[1][i] =
        inStateHex[0][i] ^ multiply2[inStateHex[1][i]]
        ^ multiply3[inStateHex[2][i]] ^ inStateHex[3][i];
      mixColumnsMatrix[2][i] =
        inStateHex[0][i] ^ inStateHex[1][i]
        ^ multiply2[inStateHex[2][i]] ^ multiply3[inStateHex[3][i]];
      mixColumnsMatrix[3][i] =
        multiply3[inStateHex[0][i]] ^ inStateHex[1][i]
        ^ inStateHex[2][i] ^ multiply2[inStateHex[3][i]];
    }
